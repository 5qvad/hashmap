...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int index = 0; index < TEXT.length(); index++) {
            char word = TEXT.toLowerCase().charAt(index);
            if (mapText.get(word) == null) {
                mapText.put(word, 1);
            } else {
                int value = mapText.remove(word);
                mapText.put(word, value + 1);
            }
        }
        char maxChar = max(mapText);
        char minChar = min(mapText);
        System.out.println("Чаще остальных встречается буква '" + maxChar + "' (" + mapText.get(maxChar) + " раз)," +
                            "реже остальных буква '" + minChar + "' (" + mapText.get(minChar) + " раз)");
